...
def getParser():
    ...
    parser.add_argument('-hour', '--checkinHour', default=8, type=int)
    parser.add_argument('-proj', '--project', default="", type=str)
    parser.add_argument('-msg', '--message', default="協助計畫執行", type=str)
    return parser
...
if __name__ == '__main__':
    ...
    try:
        ...
        requireCheckinHour = args.checkinHour
        projectName = args.project
        signoutMsg = args.message
        print("Checkin/out project name: ", projectName)
        print("Checkin/out message: ", signoutMsg)
        print("Checkin hour: ", requireCheckinHour)
        if(projectName == ""):
            print("Project name is null !")
            log.CheckinLog(f"{CURRENT_TIME} >> Project name is null ! \n")
            os._exit(0)
        else:
            log.CheckinLog(f"------------------------------------- \n")
            log.CheckinLog(f"Checkin/out project name: {projectName} \n")
            log.CheckinLog(f"Checkin/out message: {signoutMsg} \n")
            log.CheckinLog(f"Checkin hour: {requireCheckinHour} \n")
    except Exception as err:
        log.CheckinLog(f"{CURRENT_TIME} >> Load User Defined error message: {err} \n")
        print(f"{CURRENT_TIME} >> Load User Defined error message: {err}")
        os._exit(0)
    ...
    if signintime != []:
        ...
        if CURRENT_TIME.hour - signintime_hour >= requireCheckinHour:
            ...
            txt = '簽退時間未滿8小時： ' + str(CURRENT_TIME) + '\n'
            log.CheckinLog(txt)
